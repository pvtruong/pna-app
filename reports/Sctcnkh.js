...
class Sctcnkh extends React.Component {
  ...
  async componentDidMount(){
    this.mounted = true;
    try{
      let rptInfo = await asyncGetReportInfo(this.context.userInfo.token,"rSoCtCnKh");
  ...
  render() {
    ...
    return (
      <Report navigation = {this.props.navigation}
        title={getLabel("Chi tiết công nợ")}
        tableWidth={tableWidth}
        columns={[
          {id:'dien_giai',label:'Diễn giải',size:2,hideWhen:row=>!row.dien_giai && !row.so_ct,
            format:(d)=>{
              return (
                <View style={{flexDirection:'column'}}>
                  <H3 style={{fontWeight:d.bold?'bold':'normal'}}>{d.dien_giai.trim()}</H3>
                  {!!d.so_ct && <H3>Số ct: {d.so_ct}, ngày: {Moment(d.ngay_ct).format("DD/MM/YYYY")}</H3>}
                </View>
              )
            }
          }
        ...
        getUrl={condition=>{
          if(!condition.kh || !condition.kh.ma_kh){
            return {error:"Bạn chưa nhập khách hàng"}
          }
          if(!condition.tk){
          ...
          let url = `${server_url}/api/${id_app}/report/${this.state.rptInfo.code}/1?ngon_ngu=Vi&user=${this.context.userInfo.email}&ma_kh=${(condition.kh||{}).ma_kh}&tk=${condition.tk}&tu_ngay=${tu_ngay}&den_ngay=${den_ngay}`;
          return {url};
        }}
...
Sctcnkh.contextType = Context;
export default Sctcnkh
