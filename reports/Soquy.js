...
class Soquy extends React.Component {
  ...
  async componentDidMount(){
    this.mounted = true;
    try{
      let rptInfo = await asyncGetReportInfo(this.context.userInfo.token,"rsoquyth");
  ...
  render() {
    ...
    return (
      <Report navigation = {this.props.navigation}
        title={getLabel("Sổ quỹ tiền mặt")}
        ...
        getUrl={condition=>{
          ...
          let url = `${server_url}/api/${id_app}/report/${this.state.rptInfo.code}/1?ngon_ngu=Vi&user=${this.context.userInfo.email}&tai_khoan=${condition.tk}&tu_ngay=${tu_ngay}&den_ngay=${den_ngay}`;
          return {url};
        }}
