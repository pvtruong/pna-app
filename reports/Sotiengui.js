...
class Sotiengui extends React.Component {
  ...
  async componentDidMount(){
    this.mounted = true;
    try{
      let rptInfo = await asyncGetReportInfo(this.context.userInfo.token,"rsotiengui");
  ...
  render() {
    ...
    return (
      <Report navigation = {this.props.navigation}
        title={getLabel("Sổ tiền gửi")}
        tableWidth={tableWidth}
        columns={[
          {id:'dien_giai',label:'Diễn giải',size:1,hideWhen:row=>!row.dien_giai && !row.so_ct,format:(d)=>{
            return (
        ...
        getUrl={condition=>{
          if(!condition.tk){
            return {error:getLabel("Bạn chưa nhập tài khoản")};
          }
          let tu_ngay= Moment(condition.tu_ngay).format("YYYY-MM-DD");
          let den_ngay = Moment(condition.den_ngay).format("YYYY-MM-DD");
          let url = `${server_url}/api/${id_app}/report/${this.state.rptInfo.code}/1?ngon_ngu=Vi&user=${this.context.userInfo.email}&tai_khoan=${condition.tk}&tu_ngay=${tu_ngay}&den_ngay=${den_ngay}&kieu=1`;
          return {url};
        }}
...
Sotiengui.contextType = Context;
export default Sotiengui
